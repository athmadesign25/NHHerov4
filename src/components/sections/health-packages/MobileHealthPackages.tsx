...
import styles from "./MobileHealthPackages.module.css";
...
export default function MobileHealthPackages() {
  ...
  useEffect(() => {
    ...
    const applyAutoReveal = (elapsedMs: number) => {
      ...
      // Only do the left-column parallax on desktop. On mobile it's a single column layout, so translating it up overlaps the eyebrow.
      const isMob = typeof window !== 'undefined' && window.innerWidth < MOBILE_BREAKPOINT;
      if (leftCol && !isMob) {
        leftCol.style.transform = `translateY(${-34 * overallT}px)`;
      }
    ...
    const applyState = (p: number, exitP: number, mobileP1: number = 0) => {
      const frame = frameRef.current;
      const bgVideo = bgVideoRef.current;
      const grid = gridRef.current;
      const cinemaOverlay = cinemaOverlayRef.current;
      if (!frame || !bgVideo || !grid) return;

      const isMob = typeof window !== 'undefined' && window.innerWidth < MOBILE_BREAKPOINT;
      const p1raw = isMob ? mobileP1 : clamp01(p / PHASE1_END);
      const exitT = isMob ? 0 : clamp01(exitP / EXIT_DURATION);
      
      if (cinemaOverlay && isMob) {
        // Fade in as it expands, fade out as the section leaves the screen
        const mobileExitT = clamp01(exitP / 0.5); 
        cinemaOverlay.style.opacity = String((p1raw * 0.75) * (1 - mobileExitT));
      } else if (cinemaOverlay) {
      ...
      const startWidth = isMob ? 92 : 76;
      const startRadius = isMob ? 20 : 26;
      
      frame.style.width = `${startWidth + (100 - startWidth) * p1}%`;
      if (!isMob) {
        frame.style.height = `${72 + 28 * p1}%`;
      }
      const exitRadiusPx = EXIT_RADIUS_PX * exitT;
      const enterRadiusPx = startRadius * (1 - p1);
      ...
      // OTHER way p1 reaches 1 is via `isFull` snapping it there the instant
    ...
    const computeProgress = () => {
      ...
      if (!section) return { p: 0, exitP: 0, mobileP1: 0 };
      ...
      // mobileP1: goes from 0 (top of section is at bottom of screen) 
      // to 1 (top of section is at 60% of screen - i.e., 0.6 * vh)
      // distance traveled = vh - 0.6*vh = 0.4*vh
      const mobileP1 = clamp01((vh - rect.top) / (vh * 0.4));
      
      return { p, exitP, mobileP1 };
    };

    let ticking = false;
    const tick = () => {
      ticking = false;
      const { p, exitP, mobileP1 } = computeProgress();
      applyState(p, exitP, mobileP1);
    };
    const onScroll = () => {
      if (ticking) return;
      ticking = true;
  ...
  return (
    ...
    >
      <div ref={stickyViewportRef} className={styles.stickyViewport}>
        <div className={styles.cinemaOverlay} ref={cinemaOverlayRef} />
          <div ref={frameRef} className={`${styles.frame} ${textRevealed ? styles.frameRevealed : ""}`}>
                  ...
                  {TITLE_LINES.map((line, idx) => (
                    <React.Fragment key={idx}>
                      ...
                      <TextSweepEffect
                        ...
                        delayMs={TEXT_REVEAL_TITLE_DELAY * 1000 + idx * TITLE_LINE_STAGGER_MS}
                        finalColor="#ffffff"
                        active={textRevealed}
                      />
                    </React.Fragment>
