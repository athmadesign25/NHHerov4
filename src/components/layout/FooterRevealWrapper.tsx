...
// How much of main's tail the footer is allowed to rise up over. Kept well
// under AppDownloadBanner's own internal sticky-pin duration (its content
// stays static for virtually its whole ~1800px scroll track, only
// releasing in the last sliver of it), so main is guaranteed to still be
// static/pinned for the entire overlay — never "moving" underneath it.
const REVEAL_WINDOW = 480;
 ...
 * own sticky pin only releases once its internal 200vh track's true
 * bottom is reached; if this wrapper added any net height AFTER that
 * point (as pulling up by only REVEAL_WINDOW did), AppDownloadBanner would
 * release and visibly start scrolling away WHILE the footer was still
 * only partway risen. Retracting fully means AppDownloadBanner's release
 * point and the page's true scroll end are the exact same pixel — it
 * never gets the chance to visibly move, and the reveal is guaranteed to
 * finish exactly as scrolling runs out, not stall partway or overshoot.
 * Sticky positioning is bounded by its own containing block, so it can
 * only ever engage within this small window near the true end of the
 * page — on this page or any other — never earlier.
 *
